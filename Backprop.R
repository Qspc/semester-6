...
haberman.uci <- read.csv(url("https://archive.ics.uci.edu/ml/machine-learning-databases/haberman/haberman.data"))
colnames(haberman.uci)<-c("Age ","Year Operation","Positive","Status")
haberman.uci

y <- matrix(0,305,2)
y[1:152,1] <- 1
y[153:305,2] <- 1
xy <- cbind(haberman.uci[1:3],y)
xy
...
for (j in 1:3){
  xin[,j] <- (2*(xin[,j]-min(xin[,j]))/(max(xin[,j])-min(xin[,j]))) - 1
}
xin

N = 3 #jumlah input neuron
M = 8 #jumlah hidden neuron 6,7,8
L = 2 #jumlah output neuron 
alpha = 0.2 #learning rate 0.2 atau 0.4
...
yltot = matrix(0,152,2, TRUE)
yluji = matrix(0,153,2, TRUE)
...
for (i in 153:305){
  ...
  if (yl[1] != yt[i,1] || yl[2] != yt[i,2]) {
    error_uji <- error_uji + 1
  }
  yluji[i-152,] = yl
}

akurasi <- (153-error_uji)/153*100
akurasi